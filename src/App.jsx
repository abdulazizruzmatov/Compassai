...
/* ================= CONFIG — edit these ================= */
const LINKS = {
  telegram: "https://t.me/yolabroad",        // your Telegram channel
  linkedin: "https://linkedin.com/company/yolabroad", // your LinkedIn page
};
// Sponsored slot — swap for any partner university 🇺🇿
const SPONSOR = {
...
function AuthModal({ onClose }) {
  ...
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(15,29,40,0.55)", display: "grid", placeItems: "center", zIndex: 100 }}>
      <div onClick={(e) => e.stopPropagation()} className="card" style={{ width: "min(420px, 92vw)", padding: 26 }}>
        <div style={{ fontWeight: 900, fontSize: 22 }}>Sign in ✉️</div>
        ...
        {sent ? (
          ...
          <>
            ...
            <button className="btn btn-orange" style={{ width: "100%", marginTop: 12 }} onClick={send} disabled={!email.includes("@")}>Send magic link →</button>
          </>
        )}
...
function DeadlineMarquee() {
  const items = [...UNIVERSITIES, ...UNIVERSITIES]; // duplicate for seamless loop
  return (
      ...
      <div className="marquee-track">
        {items.map((u, i) => (
          <a key={i} href={u.web} target="_blank" rel="noreferrer" className="marquee-item" style={{ color: "#fff", textDecoration: "none" }}>
            <span style={{ color: "var(--orange)", fontWeight: 700 }}>{u.qs ? `QS #${u.qs}` : "NEW"}</span>
            <span style={{ fontWeight: 600 }}>{u.name}</span>
            <span style={{ color: "#9FB0BC" }}>{u.country}</span>
            <span style={{ color: "#9FB0BC" }}>⏰ {u.deadline}</span>
          </a>
        ))}
      </div>
    </div>
...
/* ---------------- Boarding pass (AI result card) ---------------- */
function BoardingPass({ u, home, idx, tracked, onTrack }) {
  const isTracked = tracked.some((t) => t.uni?.name === u.name || t.name === u.name);
  return (
    <div className="card" style={{ overflow: "hidden", boxShadow: "0 2px 10px rgba(15,29,40,0.07)", animation: `rise .5s ${idx * 0.08}s both ease-out` }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 18px", background: "var(--ink)", color: "#fff" }}>
        <div className="mono" style={{ fontSize: 20, fontWeight: 700, letterSpacing: "0.05em" }}>
          {code3(home)} <span style={{ color: "var(--orange)" }}>→</span> {code3(u.city)}
        </div>
        <div className="mono" style={{ fontSize: 11, letterSpacing: "0.12em", color: "#9FB0BC", textTransform: "uppercase" }}>{u.rank || "Ranked"}</div>
      </div>
      <div style={{ padding: "16px 18px 6px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "flex-start" }}>
          <div>
            <div style={{ fontWeight: 800, fontSize: 19, lineHeight: 1.2 }}>{u.name}</div>
          ...
          <button onClick={() => onTrack(u)} disabled={isTracked} style={{
            ...
            fontWeight: 800, fontSize: 12.5, fontFamily: "Archivo, sans-serif",
        ...
        <div style={{ marginTop: 14, padding: "10px 12px", background: "#F2F5F7", borderRadius: 8, fontSize: 13.5 }}>
          <span className="mono" style={{ fontSize: 11, letterSpacing: "0.12em", color: "var(--orange)", fontWeight: 700 }}>🛂 VISA </span>{u.visa}
        </div>
      </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 18px 16px", flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", gap: 8, flexShrink: 0, flexWrap: "wrap" }}>
          <a className="btn btn-orange" style={{ padding: "10px 14px", fontSize: 13 }} href={u.apply || u.web} target="_blank" rel="noreferrer">Apply now ↗</a>
...
function GlobeSection() {
  ...
  return (
    <section id="globe" className="section" style={{ background: "var(--ink)", color: "#fff" }}>
      <div className="container">
        <h2 className="section-title">Spin the globe 🌍</h2>
        <p className="section-sub" style={{ color: "#9FB0BC" }}>Drag to turn it, tap a marker — see estimated rent, food, transport and the visa you'd need there.</p>
          ...
          <div>
            {picked ? (
              <div className="card" style={{ padding: 22, color: "var(--ink)", animation: "rise .4s both" }}>
                <div style={{ fontWeight: 900, fontSize: 24 }}>{picked.name}</div>
                ...
                <div style={{ marginTop: 16, padding: "10px 12px", background: "#F2F5F7", borderRadius: 8, fontSize: 13.5 }}>
                  <span className="mono" style={{ fontSize: 11, color: "var(--orange)", fontWeight: 700 }}>🛂 VISA </span>{picked.visa}
                </div>
                <div style={{ marginTop: 10, fontSize: 14, color: "var(--green)", fontWeight: 700 }}>💡 {picked.note}</div>
              </div>
            ) : (
              <div style={{ border: "1.5px dashed #3C4E5C", borderRadius: 14, padding: "40px 24px", textAlign: "center", color: "#9FB0BC" }}>
                👆 Tap an orange marker to see costs for that country
              </div>
            )}
          </div>
        </div>
        <div style={{ fontSize: 12, color: "#7C8D9A", marginTop: 16 }}>Estimates for students, 2026 — always double-check official sources.</div>
      </div>
    </section>
  );
}
...
export default function App() {
  ...
  /* ----- tracker load: Supabase if signed in, else localStorage ----- */
  useEffect(() => {
    (async () => {
      if (supabase && session) {
        ...
        try { setTracked(JSON.parse(localStorage.getItem("yol-tracker") || "[]")); } catch { setTracked([]); }
      }
    })();
  }, [session]);

  const persistLocal = (list) => localStorage.setItem("yol-tracker", JSON.stringify(list));

  const track = async (u) => {
    if (tracked.some((t) => (t.uni?.name || t.name) === u.name)) return;
    if (supabase && session) {
      const { data, error: e } = await supabase.from("tracked_applications")
        .insert({ user_id: session.user.id, uni: u, status: "Not started", notes: "" }).select().single();
      if (!e && data) setTracked((p) => [...p, data]);
    } else {
  ...
  return (
    <div>
      ...
      {/* ============ NAV ============ */}
      <header style={{ position: "sticky", top: 0, zIndex: 50, background: "rgba(251,252,253,0.92)", backdropFilter: "blur(8px)", borderBottom: "1px solid var(--line)" }}>
        <div className="container" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 20px" }}>
          <div style={{ fontWeight: 900, fontSize: 20, letterSpacing: "-0.02em" }}>YO'L<span style={{ color: "var(--orange)" }}>.</span> <span style={{ fontSize: 12, color: "var(--slate)", fontWeight: 600 }}>study abroad 🌍</span></div>
          <nav className="nav-links" style={{ display: "flex", gap: 20, fontSize: 14, fontWeight: 700 }}>
            {[["advisor", "🧭 Advisor"], ["scholarships", "🎓 Scholarships"], ["globe", "🌍 Globe"], ["tracker", "📋 My Tracker"]].map(([id, l]) => (
              <a key={id} href={`#${id}`} onClick={(e) => { e.preventDefault(); scrollTo(id); }} style={{ textDecoration: "none" }}>{l}</a>
            ))}
          </nav>
      ...
      {/* ============ STATS STRIP ============ */}
      <div style={{ background: "var(--ink)", color: "#9FB0BC" }}>
        <div className="container mono" style={{ display: "flex", gap: 26, padding: "10px 20px", fontSize: 12, flexWrap: "wrap" }}>
          <span>🟢 Live</span><span>🏛️ 1,000+ universities</span><span>🎓 {SCHOLARSHIPS.length} major scholarships</span><span>🌍 {COUNTRY_COSTS.length} countries mapped</span><span>💸 $0 consultant fees</span>
        </div>
      </div>

      {/* ============ HERO ============ */}
      <section className="section" style={{ textAlign: "center", padding: "72px 0 56px" }}>
        <div className="container">
          <div className="mono" style={{ fontSize: 12, letterSpacing: "0.18em", color: "var(--orange)", fontWeight: 700 }}>🇺🇿 → 🌍 FOR STUDENTS EVERYWHERE</div>
          <h1 className="hero-h1" style={{ fontWeight: 900, fontSize: 62, lineHeight: 1.0, letterSpacing: "-0.03em", margin: "14px auto 14px", maxWidth: 760 }}>
            Stop paying consultants.<br />Ask <span style={{ color: "var(--orange)" }}>who you want to be</span>.
          </h1>
          <p style={{ color: "var(--slate)", fontSize: 17, maxWidth: 580, margin: "0 auto 28px", lineHeight: 1.55 }}>
            An AI advisor that turns "I want to be a game developer 🎮" into real universities, scholarships, living costs, visas and direct application links — free.
          </p>
          <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
            <button className="btn btn-orange" style={{ fontSize: 17, padding: "17px 28px" }} onClick={() => scrollTo("advisor")}>🧭 Who do you wanna be? →</button>
            <button className="btn btn-ghost" onClick={() => scrollTo("scholarships")}>Browse scholarships 🎓</button>
          </div>
        </div>
      </section>
      ...
      <section id="advisor" className="section">
        <div className="container">
          <h2 className="section-title">Your AI advisor 🧭</h2>
          <p className="section-sub">Describe yourself like you'd tell a friend. The AI builds your direction, matches universities, and you track them — all in one place.</p>

          {phase === "form" && (
            <div className="card" style={{ padding: "22px 22px 26px", boxShadow: "0 2px 10px rgba(15,29,40,0.06)" }}>
              ...
              <div style={{ display: "grid", gap: 18, marginTop: 10 }}>
                ...
                {error && <div style={{ color: "var(--orange)", fontWeight: 700, fontSize: 14 }}>{error}</div>}
                <button className="btn btn-orange" style={{ padding: "16px 20px", fontSize: 16 }} onClick={run} disabled={!ready}>BUILD MY PLAN →</button>
              </div>
            </div>
          )}
          ...
          {phase === "loading" && (
            <div style={{ textAlign: "center", padding: "90px 0" }}>
              ...
              <div style={{ fontWeight: 900, fontSize: 34, marginTop: 14 }}>Plotting your route <span style={{ color: "var(--orange)" }}>→</span></div>
            </div>
          )}
          ...
          {phase === "results" && plan && (
            <div style={{ display: "grid", gap: 18 }}>
              <div style={{ background: "var(--ink)", color: "#fff", borderRadius: 14, padding: "20px 22px", animation: "rise .5s both" }}>
                <div className="mono" style={{ fontSize: 11, letterSpacing: "0.16em", color: "var(--orange)" }}>🎯 YOUR DIRECTION</div>
                <div style={{ fontWeight: 900, fontSize: 28, margin: "6px 0 8px" }}>{plan.title}</div>
                <div style={{ fontSize: 14.5, lineHeight: 1.55, color: "#D7DFE5", maxWidth: 640 }}>{plan.summary}</div>
                {plan.degrees?.length > 0 && (
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 14 }}>
                    {plan.degrees.map((d, i) => <span key={i} className="mono" style={{ border: "1.5px solid #3C4E5C", borderRadius: 8, padding: "5px 10px", fontSize: 12.5 }}>{d}</span>)}
                  </div>
                )}
              </div>

              {unis.map((u, i) => <BoardingPass key={u.name + i} u={u} home={form.home} idx={i % 5} tracked={tracked} onTrack={track} />)}
              {error && <div style={{ color: "var(--orange)", fontWeight: 700, fontSize: 14, textAlign: "center" }}>{error}</div>}
      ...
      {/* ============ SCHOLARSHIPS ============ */}
      <section id="scholarships" className="section" style={{ background: "#E2E9EE" }}>
        <div className="container">
          <h2 className="section-title">Scholarships around the world 🎓</h2>
          ...
          <div className="grid3">
            {SCHOLARSHIPS.map((s) => (
              <a key={s.name} href={s.url} target="_blank" rel="noreferrer" className="card" style={{ padding: 18, textDecoration: "none", display: "block" }}>
                ...
                <div style={{ fontWeight: 800, fontSize: 16, marginTop: 6 }}>{s.name}</div>
                ...
                <div className="mono" style={{ fontSize: 12, color: "var(--orange)", fontWeight: 700, marginTop: 8 }}>⏰ {s.deadline}</div>
              </a>
            ))}
          </div>
        </div>
      ...
      <section className="section" style={{ paddingBottom: 0 }}>
        <div className="container">
          <div className="card" style={{ padding: 24, display: "flex", gap: 20, alignItems: "center", flexWrap: "wrap", borderLeft: "4px solid var(--orange)" }}>
            <div style={{ flex: 1, minWidth: 240 }}>
              <div className="mono" style={{ fontSize: 10, letterSpacing: "0.16em", color: "var(--slate)" }}>SPONSORED 🤝</div>
              <div style={{ fontWeight: 900, fontSize: 22, marginTop: 4 }}>{SPONSOR.name}</div>
            ...
            <a className="btn btn-orange" href={SPONSOR.url} target="_blank" rel="noreferrer">{SPONSOR.cta} ↗</a>
          </div>
        </div>
      </section>
        ...
        <div className="container">
          ...
          <div className="grid4" style={{ marginBottom: 18 }}>
            {[["Tracked", counts.total, "var(--ink)"], ["In progress", counts.progress, "var(--amber)"], ["Applied", counts.applied, "var(--blue)"], ["Offers 🎉", counts.offers, "var(--green)"]].map(([l, n, c]) => (
              <div key={l} className="card" style={{ padding: "14px 16px", textAlign: "center" }}>
                <div style={{ fontWeight: 900, fontSize: 30, color: c }}>{n}</div>
          ...
          {tracked.length === 0 ? (
            <div style={{ border: "1.5px dashed var(--line)", borderRadius: 14, padding: "50px 24px", textAlign: "center", background: "var(--card)" }}>
              <div style={{ fontWeight: 800, fontSize: 18 }}>Nothing tracked yet 👀</div>
              <div style={{ color: "var(--slate)", fontSize: 14, margin: "8px 0 18px" }}>Build your plan above, then press ⭐ Track on any university.</div>
              <button className="btn btn-orange" onClick={() => scrollTo("advisor")}>Find my path →</button>
            </div>
          ) : (
              ...
              {[...tracked].sort((a, b) => ((a.deadline || "9999") < (b.deadline || "9999") ? -1 : 1)).map((t) => {
                ...
                return (
                  <div key={t.id} className="card" style={{ borderLeft: `4px solid ${c}`, padding: "14px 16px" }}>
                    ...
                    {t.deadline && <div className="mono" style={{ fontSize: 12, color: "var(--orange)", fontWeight: 700, marginTop: 8 }}>⏰ DEADLINE {t.deadline}</div>}
      ...
      {/* ============ COMMUNITY ============ */}
      <section className="section" style={{ background: "#E2E9EE", textAlign: "center" }}>
        <div className="container">
      ...
      {/* ============ FOOTER ============ */}
      <footer style={{ background: "var(--ink)", color: "#9FB0BC", padding: "26px 0" }}>
        <div className="container" style={{ display: "flex", justifyContent: "space-between", flexWrap: "wrap", gap: 12, fontSize: 13 }}>
          <div><span style={{ color: "#fff", fontWeight: 900 }}>YO'L<span style={{ color: "var(--orange)" }}>.</span></span> — the way abroad 🌍</div>
